...
const HOME_ROUTE = '/vendor/dashboard';
const LOGIN_ROUTE = '/vendor';
...
export default function withVendorAuth<T extends WithAuthProps = WithAuthProps>(
    ...
    React.useEffect(() => {
      if (!isLoading) {
        if (isAuthenticated) {
          ...
          if (routeRole === 'auth' && user?.type !== 'vendor') {
            if (query?.redirect) {
